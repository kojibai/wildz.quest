...
[![Receiz SDK](https://img.shields.io/badge/Receiz%20SDK-118.0.0-6f42c1.svg)](https://www.npmjs.com/package/@receiz/sdk)
...
| `@receiz/sdk@118.0.0` | Identity, proof objects, custody, ownership, publication, audit, and settlement clients | The typed application/runtime boundary |
| `@receiz/mcp-server@118.0.0` | Capability inspection and authorized operator workflows | Tooling only; it cannot manufacture proof or authority |
| `@receiz/ai-skills@118.0.0` plus the checked-in Wildz skills | Proof-aware build, market, and release procedures for coding agents | Operating guidance only; verification still wins |

All three Receiz packages resolve at exact version `118.0.0` from the public npm registry, with their published SHA-512 integrity values pinned by `pnpm-lock.yaml`. The application contract in [`receiz.app.json`](receiz.app.json) selects artifact-first authority and explicitly disables database authority.

V118 coordinates the SDK, MCP server, AI skills, ruleset, registry, operation matrix, compatible package range, and packed runtime as one release identity. Durable proof memory remains first admission only, then append forever. Deferred v119 orchestration is not shipped.
...
That command runs the Node test suite, typecheck, Receiz v118 contract checker, MCP conformance, lint, tracked/untracked text secret scan, production build, and default Receiz doctor.
...
Application-facing Receiz code lives in [`src/lib/receiz`](src/lib/receiz). UI and game modules consume these adapters instead of scattering SDK calls across components. New v118 artifacts use the native Record → Seal flow, preserve SDK-returned bytes exactly, and are independently reopened before acceptance.

The checked-in contract and generated evidence bind the application to the v118 ruleset, registry digest, 16-operation matrix, protocol limits, 30 numbered artifact laws, profile/economy showcase authority, and native-capture/PBI-authorship rules. The matrix retains profile-showcase genesis and append planning plus economy-showcase genesis, append, and merge planning while keeping plans non-authoritative and zero-write:
...
Public reads do not require a bearer token. Authorized delegated operations require a scoped Receiz credential supplied to the MCP process. MCP packages must stay out of browser and application bundles. See [`docs/MCP.md`](docs/MCP.md) for the supported v118 inventory and credential boundary.
...
The current v118 SDK does not expose Wildz's required conditional market-ownership append. Those live mutations therefore remain deliberately unavailable; the app does not substitute IndexedDB, process memory, or a successful checkout response for settlement authority.
