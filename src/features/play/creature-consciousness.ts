 ...
 * used only when both genuine Receiz inference rails fail, and never invents a
 * world event or changes canonical state.
 */
function proofGroundedCreatureReplyText(brain: CreatureBrainProjection, message: string) {
...
export function proofGroundedCreatureReply(brain: CreatureBrainProjection, message: string) {
  return normalizeCreatureSpokenPerspective(proofGroundedCreatureReplyText(brain, message), brain.identity.name);
}
