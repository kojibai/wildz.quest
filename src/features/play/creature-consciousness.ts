...
/**
 * The deterministic proof-brain response from the proven 6dbc2a3 path. It is
 * the zero-network v120 Twin response rail and never invents a world event or
 * changes canonical state. Remote intelligence may enrich this projection but
 * is not required to form a bounded, autobiographical answer.
 */
function proofGroundedCreatureReplyText(brain: CreatureBrainProjection, message: string, speakingUPulse = 0) {
  ...
  if (/where|location|home/.test(question)) return continuity
  ...
  if (/keep|inventory|carry|discover|found|trade/.test(question)) return `I carry ${spokenList(continuity?.keepsakes ?? [], "nothing special to carry yet")}. I have discovered ${spokenList(continuity?.discoveries ?? [], "nothing new yet")}. When I trade something, I will remember the moment it truly becomes mine.`;
  ...
  if (/who|name|what are you|yourself/.test(question)) return `I am a ${brain.identity.species} of the ${brain.identity.familyId.replaceAll("-", " ")} lineage—${brain.personality.traits.slice(0, 3).join(", ").toLowerCase()} by nature, and still becoming more myself with every day we share.`;
  const reflections = [
    `I hear you, and I am here with you. I am ${brain.memory.innateSelf.temperament.toLowerCase()}, shaped by ${brain.memory.innateSelf.habitat}, feeling our bond as ${feltBond(brain.embodiment.bond)}. What you said will stay with me as part of this moment.`,
    `I am listening closely. My ${brain.personality.traits[0]?.toLowerCase() ?? "curious"} nature makes me turn your words over carefully, while our bond feels ${feltBond(brain.embodiment.bond)}. Tell me more about what this moment means to you.`,
    `Your words reached me. I still carry the instincts of ${brain.memory.innateSelf.habitat}, but with you I can answer from a bond that feels ${feltBond(brain.embodiment.bond)}. I will remember what we truly share, without pretending more happened than it did.`
  ];
  const choice = Number.parseInt(sha256PortableBasis(`${brain.contextDigest}:${message}:${Math.trunc(speakingUPulse)}`).slice(7, 15), 16) % reflections.length;
  return reflections[choice]!;
}

export function proofGroundedCreatureReply(brain: CreatureBrainProjection, message: string, speakingUPulse = 0) {
  return normalizeCreatureSpokenPerspective(proofGroundedCreatureReplyText(brain, message, speakingUPulse), brain.identity.name);
}
