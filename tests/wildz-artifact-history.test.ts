...
test("complete verified artifacts are retained exactly and duplicate admission is idempotent", async () => {
  ...
  assert.equal(first.schema, "receiz.wildz.artifact_history.v119");
...
test("v108 history remains idempotent when the same verified artifact is admitted under v119", async () => {
