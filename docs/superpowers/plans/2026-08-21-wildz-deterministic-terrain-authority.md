...
import assert from "node:assert/strict";
import { test } from "node:test";
import { buildWildsObstacleIndex, queryWildsObstacles, wildsTerrainObstaclesForTile } from "../src/features/play/wilds-terrain-obstacles";

test("physical obstacles are deterministic stable records", () => {
  const first = wildsTerrainObstaclesForTile(6, -4);
  assert.deepEqual(first, wildsTerrainObstaclesForTile(6, -4));
...
test("arrival, routes, and landmark aprons contain no generated solid obstacle", () => {
  const originTiles = [-1, 0, 1].flatMap((tileX) => [-1, 0, 1].flatMap((tileZ) => wildsTerrainObstaclesForTile(tileX, tileZ)));
  assert.equal(originTiles.some((obstacle) => Math.hypot(obstacle.position.x, obstacle.position.z) < 15), false);
});

test("spatial queries return only intersecting records in stable id order", () => {
  const obstacles = wildsTerrainObstaclesForTile(10, 10);
  const index = buildWildsObstacleIndex(obstacles);
  const result = queryWildsObstacles(index, { minX: 120, maxX: 126, minZ: 120, maxZ: 126 });
  assert.deepEqual(result, [...result].sort((left, right) => left.id.localeCompare(right.id)));
  assert.ok(result.every((obstacle) => obstacle.position.x + obstacle.radius >= 120 && obstacle.position.x - obstacle.radius <= 126));
});
```
