...
test("complete verified artifacts are retained exactly and duplicate admission is idempotent", async () => {
  ...
  assert.equal(first.schema, "receiz.wildz.artifact_history.v116");
...
test("v108 history remains idempotent when the same verified artifact is admitted under v116", async () => {
