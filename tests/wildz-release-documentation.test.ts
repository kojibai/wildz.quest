...
test("Wildz current release doctrine names the exact Receiz v124 toolchain", () => {
  ...
  assert.equal(pkg.dependencies?.["@receiz/sdk"], "124.0.2");
  assert.equal(pkg.devDependencies?.["@receiz/mcp-server"], "124.0.2");
  assert.equal(pkg.devDependencies?.["@receiz/ai-skills"], "124.0.2");
  for (const [name, source] of Object.entries(currentDoctrine)) {
    for (const packageName of ["@receiz/sdk", "@receiz/mcp-server", "@receiz/ai-skills"]) {
      assert.match(source, new RegExp(`${packageName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}@124\\.0\\.2`), name);
    }
  ...
  assert.match(mcp, /@receiz\/sdk@124\.0\.2/);
  assert.match(mcp, /@receiz\/mcp-server@124\.0\.2/);
  assert.match(mcp, /@receiz\/ai-skills@124\.0\.2/);
