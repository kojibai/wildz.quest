...
- Validator: `docs/release/evidence/final-integration/validate-final-integration-evidence.mjs` — SHA-256 `6283dbc3f61b54826672eaf7a4e4ad9e878b5392c9c070d277e3515f72ada9de`; executable tracked mode `100755`.
...
Exact commands are retained in `manifest.commands`: `pnpm start -p 49816`; cached Playwright CLI fresh-session open; saved `run-code` replay from the tracked script; and `node docs/release/evidence/final-integration/validate-final-integration-evidence.mjs`. The replay deliberately writes a new capture under `output/playwright/`; the tracked JSON and screenshots remain the immutable recorded run. `manifest.capturePathMap` resolves all seven raw path-bearing result fields—the script, result, and five screenshots—to their tracked copies. The validator hard-codes the independent expected literals for browser metadata, timestamps, request count, Profile's exact four-label sequence repeated three times, focus labels, every arrow's coordinates/descendants/named selection, exact before/after state and deltas, pointer transition order, and owner-cancellation fields. It also enforces mapping completeness, product-commit ancestry, Git tracking and mode, SHA-256, and byte size. JSON parsing, the validator, and every manifest hash/size check passed from the repository root.
