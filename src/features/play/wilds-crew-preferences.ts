...
export function sanitizeWildsCrewPreferences(value: unknown, inventory: readonly PortableCardAsset[], ownerReceizId?: string): WildsCrewPreferences | undefined {
  ...
  const entries = inventory.filter(asset => sameWildzPlayerCoordinate(asset.manifest.ownerReceizId, ownerReceizId))
    .flatMap(asset => {
...
export function setWildsCrewPreference(value: WildsCrewPreferences | undefined, inventory: readonly PortableCardAsset[], ownerReceizId: string, assetId: string, mode: WildsCrewMode): WildsCrewPreferences | undefined {
  if ((mode !== "follow" && mode !== "roam") || !inventory.some(asset => asset.id === assetId && sameWildzPlayerCoordinate(asset.manifest.ownerReceizId, ownerReceizId))) return value;
  const current = sanitizeWildsCrewPreferences(value, inventory, ownerReceizId);
