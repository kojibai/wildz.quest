...
test("authenticated v103 native export submits only the validated PNG and returns the Record/Seal artifact byte-exact", async () => {
  ...
  assert.deepEqual({
    ...
    filename: "wilds-vault.png",
    idempotencyKey: `wildz-v118-${await crypto.subtle.digest("SHA-256", source.slice().buffer).then((value) => Buffer.from(value).toString("hex"))}`
  });
...
test("v118 export rejects native continuity that does not match the authenticated owner or verifier verdict", async () => {
  const source = vaultPng();
