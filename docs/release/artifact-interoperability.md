...
This document defines the v3/v103 interoperability contract and names the local focused evidence. External writer, strict-live, and remote mutation qualification remain separate gates and are not inferred from package installation or fixtures.

## Authority order

1. The official Receiz v103 verifier establishes cryptographic proof status for the exact enclosing artifact.
...
The player OIDC session requests the v103 proof-store Record/Seal/Verify scopes `receiz:record`, `receiz:seal`, and `receiz:verify` because native card and Vault creation uses that authenticated rail. `offline_access` is not requested, and refresh tokens are not transported or retained by Wildz.
...
For v103 creation, the SDK-returned native artifact is the final download and is preserved byte-identical with its returned MIME type. For legacy v102 input, the compatibility reader validates canonical structure, payload digest, bundle basis, and proof claim before exposing the original payload bytes. PNG Wildz payloads and identity trailers remain exact inputs to their respective verifiers.
...
| `sdk-compatible` | Local SDK v103 contract coverage; external exercise pending |
