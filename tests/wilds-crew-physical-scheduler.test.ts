...
it("roundrobins beyond route cache capacity without dropping physical positions or starving entries",()=>{
 ...
 for(let i=0;i<20;i++){const result=scheduler.tick();assert.ok(result.processed<=2);assert.ok(result.plans<=1);assert.ok(result.cachedRoutes<=12);}
 assert.equal(runtime.size,20);
 for(const value of runtime.values()){assert.ok(value.position!.x>0);assert.ok(value.position!.x<=1.1+.000001);}
});
it("hands party movement to exactly one writer and resumes from its last actual position",()=>{
 ...
 scheduler.tick();assert.equal(value.position!.x,0);
 value.position!.x=3;party=false;scheduler.tick();assert.ok(value.position!.x>3&&value.position!.x<=3.55);
 party=true;const prior={...value.position!};scheduler.tick();assert.deepEqual(value.position,prior);
});
it("pauses unavailable geometry/readiness and explicit halted trips without inventing arrival",()=>{
 ...
 scheduler.tick();assert.equal(value.paused,true);assert.equal(value.position!.x,0);
 loaded=true;value.halted=true;scheduler.tick();assert.equal(value.position!.x,0);
 value.halted=false;scheduler.tick();assert.equal(value.paused,false);assert.ok(value.position!.x>0);
});
it("refuses missing dispatch coordinates and unsupported movement modes",()=>{
 ...
 scheduler.tick();assert.equal(value.position,null);assert.equal(value.paused,true);
 value.position={x:0,y:0,z:0};scheduler.tick();assert.equal(value.position.x,0);
});
it("commits a physical obstacle detour with bounded plans and never crosses the blocker",()=>{
 ...
 for(let i=0;i<100;i++){const before={...value.position!};scheduler.tick();const out={allowed:false,y:NaN};blocked.sampleSegment(before,value.position!,"walk",out);assert.equal(out.allowed,true);detoured ||= Math.abs(value.position!.z)>.7;}
...
it("target changes and long scheduling gaps never teleport or overshoot",()=>{
 const value=entry(),runtime=new Map([["creature",value]]);
 const scheduler=createWildsCrewPhysicalScheduler({runtime:()=>runtime,admit:()=>clear});scheduler.tick();
 value.target={x:-10,y:0,z:0};const before=value.position!.x;scheduler.tick();assert.ok(Math.abs(value.position!.x-before)<=.55+.00001);
 value.target={x:value.position!.x+.01,y:0,z:0};scheduler.tick();assert.equal(value.position!.x,value.target.x);
});
