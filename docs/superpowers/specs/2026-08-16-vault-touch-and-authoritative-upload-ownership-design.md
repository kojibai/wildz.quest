...
- The server validates the complete artifact against the authoritative current ownership head. A historical artifact whose card has since moved to another owner is rejected and cannot reclaim the card.
- A successful claim returns a newly sealed artifact with the uploader appended as the ownership head. The client verifies and restores those returned bytes, never the stale uploaded bytes, and downloads the new artifact for the owner.
...
Keep the current upload control and import summary. Route each `merge-vault` upload through `/api/market/claims` while leaving `activate-identity` unchanged. The existing claim route and Receiz SDK remain the sole transfer authority:

1. The server resolves the authenticated actor and asks Receiz to claim the complete artifact.
2. Receiz rejects a stale ownership head with the existing stale-custody error.
3. The server records the admitted ownership projection and returns the new sealed artifact.
...
- A market authority outage never guesses ownership or deletes a local card; reconciliation resumes when authority is available.
...
- Route and policy tests prove reconciliation is authenticated, bounded, returns only foreign-owned asset IDs, and fails closed when authority is unavailable.
