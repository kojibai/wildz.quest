...
|---|---|---|---|
| `@receiz/sdk` | `111.0.0` | `111.0.0` | Application identity, artifact, native proof-object, constitutional compiler, command admission, causal replay, checker, and remote-rail client |
| `@receiz/mcp-server` | `111.0.0` | `111.0.0` | Operator tooling; never application authority |
| `@receiz/ai-skills` | `111.0.0` | `111.0.0` | V111 operator procedure guidance; never proof authority |

The finalized v111 SDK, MCP, and AI-skills packages resolve directly from the official npm registry, with their published integrity values pinned by the lockfile. `receiz.app.json` declares artifact-first authority with database authority disabled; the v111 compiler and `pnpm receiz:check` validate the repository integration plan, release identity, ruleset, and registry digest `cf02d0bce6ad1541cfe84e27bfb1036777b29616bf8a1e5aeafb899a945e359a`.

## Local evidence

| Gate | Result |
|---|---|
| `pnpm release:check` | Pass: v111 repository integration and release-identity checker, complete Node test suite, typecheck, lint, full tracked/untracked text secret scan, production build, and default Receiz doctor |
| Historical migration compatibility | Pass: forward-only checkpoint retained; sealed artifacts, receipts, and heads preserved; current execution remains v111-only |
| V111 offline authority | Pass: release identity asserts that a queued proposal is not a global commitment |
...
| Native proof-object continuity | Pass: v111 Record → Seal artifact, owner, record, claim, verify path, Signature V4, exact download digest, and exact-file reopen; `wildz-v111` retries bind to the payload digest |
| Unified artifact recovery | Pass: v111 admission, zero-write recovery planning, verified-capability commit, atomic store boundary, and MCP/AI operation parity are exposed and law-pinned |
...
| Market settlement coordinator | Pass with local contract doubles: admitted trade, Receiz Connect transfer proof, corroborating wallet ledger event, conditional ownership append, and idempotent recovery; v111 exposes no Wildz-specific conditional append, so the live adapter remains fail-closed without that capability |
