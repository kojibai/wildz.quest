...
## v110 application contract, registry, and checker

`receiz.app.json` defines Wildz as a v110 `receiz.app.contract.v1` application with the `identity`, `proof`, `proofMemory`, `publicStore`, and `commerce` features. It selects `artifact-first` authority and sets `allowDatabaseAuthority` to `false`. The SDK's compiler APIs compile that declaration into a deterministic integration plan; the checked-in `receiz.generated.json` records repository evidence for Record-before-Seal, durable proof memory, continuity verification, idempotency, and browser-safe secrets. Constitutional operations bind to registry digest `824aa4af849c4840ba94535798eab36e45d514703b6ae0cd30d4aa53f3c896e4`.

Run `pnpm receiz:check` to invoke the official v110 repository checker against target `110.0.0` and verify the exact release, ruleset, and registry digest. The command is also part of the release gate. A clean checker result confirms that the declared repository integration requirements have evidence; it is not a substitute for artifact verification, strict-live qualification, or remote mutation evidence.

The checked-in historical migration checkpoint remains forward-only evidence: no sealed artifact, receipt, or proof head was rewritten. The current runtime and checker are v110-only. Historical signed-command mechanics are not current outcomes, and a queued proposal is not a global commitment.

## v110 artifact operation boundary

Current v110 artifact outcomes include `assets.createProofObject`, `artifacts.download`, `artifacts.verifyAndOpen`, `artifacts.admit`, `artifacts.planRecovery`, `artifacts.admitAndRecover`, `artifacts.commitRecovery`, and `ownership.claimBearerAsset`. Admission and recovery planning are zero-write projections. A recovery commit requires an SDK-verified capability and an atomic caller-provided store; neither a plan nor its explanation is proof authority. Historical key/head/receipt/reconcile mechanics are not current defaults. Wildz does not infer ownership, publication, or settlement from a local plan, emulator result, compatibility response, registry payload, or queued proposal.

The SDK v110 commerce clients do not expose the Wildz-specific verified conditional market ownership append required by the game contract. The market adapter therefore reports missing capability and fails closed when that append is unavailable. MCP must not synthesize it, and IndexedDB or process memory must not be presented as durable market authority.

## v110 native proof objects

New card and Vault exports use the SDK v110 native Record → Seal operation. Wildz supplies only the artifact type and exact payload bytes. The authenticated Receiz service resolves owner, claim, verification path, and native continuity; application code does not author ownership, namespace, provenance, settlement, or prior-head authority.
...
The SDK-returned native artifact is the final download. Its bytes and MIME type are preserved exactly; Wildz never wraps a new v110 artifact in a legacy portable-asset envelope. The `wildz-v110` idempotency namespace binds retries to the exact payload digest.

## Legacy v102 read compatibility

Existing `receiz.portable_asset.v1` artifacts remain importable only after v110 `artifacts.verifyAndOpen` returns `verified-legacy-read`. Wildz may parse the returned `verifiedPayload.bytes`; it never parses the enclosing artifact itself. Compatibility is read-only and isolated from all current proof creation.
