  ...
  return <group name="continuous-construction" position={[-player.x, -terrainElevation, -player.z]}>
    {pieces.filter(({ component }) => (component.evidence.spaceId??"wildz.space.outer.v1")===spaceId && Math.hypot(component.transform.position.x - player.x, component.transform.position.z - player.z) <= 64).map(({ component, geometry }) => {
      ...
      return <group key={component.componentId} name={`construction-stage-${geometry.stage}`} visible={!(preview&&activeComponentId===component.componentId)} onClick={select}
        onPointerDown={event=>{if(!selectable||!onDrag||event.button!==0||drag.current)return;suppressClick.current=false;event.stopPropagation();if(controls){controlsEnabled.current=controls.enabled;controls.enabled=false;}const origin={x:component.evidence.pointer.x,z:component.evidence.pointer.z,surfaceWorldY:component.evidence.pointer.y};drag.current={id:component.componentId,pointerId:event.pointerId,clientX:event.clientX,clientY:event.clientY,start:event.point.clone(),origin,last:origin,plane:new THREE.Plane(new THREE.Vector3(0,1,0),-event.point.y),moved:false};(event.target as Element).setPointerCapture(event.pointerId);}}
