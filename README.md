...
[![Receiz SDK](https://img.shields.io/badge/Receiz%20SDK-126.0.0-6f42c1.svg)](https://www.npmjs.com/package/@receiz/sdk)
...
| `@receiz/sdk@126.0.0` | Identity, proof objects, durable subjects, authority sessions, durable execution, recipient resolution, and executable Phi value rails | The typed application/runtime boundary |
| `@receiz/mcp-server@126.0.0` | Capability inspection and authorized operator workflows | Tooling only; it cannot manufacture proof or authority |
| `@receiz/ai-skills@126.0.0` plus the checked-in Wildz skills | Proof-aware build, market, and release procedures for coding agents | Operating guidance only; verification still wins |

All three Receiz packages resolve at exact version `126.0.0` from the public npm registry, with their published SHA-512 integrity values pinned by `pnpm-lock.yaml`. This release adopts the v126 ruleset, whose registry digest is `80137c2e6f294050ef36ff75e4daac15c7790b7f04d9a91fab9d1970fa3c0b09` and operation-matrix digest is `42c7f0924df91b4ba11c1b891fee2b92abb509430a86b030735c23d055e67949`. The application contract in [`receiz.app.json`](receiz.app.json) selects artifact-first authority, declares the complete 60-operation V126 matrix, and explicitly disables database authority. See the [V124 runtime composition](docs/RECEIZ_V124_RUNTIME.md) for qualified Twin enrichment, proof memory, authority sessions, durable execution, replay/private additions, and MCP custody boundaries.
...
- Universal playable proof claims for exact card custody today and SDK-issued Phi, resource, access, and world-right transition sets through the same native Claim surface.
...
That command runs the Node test suite, typecheck, Receiz v126 contract checker, MCP conformance, lint, tracked/untracked text secret scan, production build, and default Receiz doctor.
...
The checked-in contract and generated evidence bind the application to the v126 ruleset, registry digest, 60-operation matrix, protocol limits, retained numbered artifact laws, living-subject authority, profile/economy showcases, and native-capture/PBI-authorship rules. Twin output is non-authoritative; factual memory cites admitted events; multi-subject effects are atomic; and bearer transfer preserves subject identity while revoking the former owner.
