...
export function renderEyes(identity: HeartboundIdentityV2, palette: HeartboundPalette) {
  ...
  return `<g data-eye-style="${face.signature}" transform="rotate(${face.eyeTilt} 280 ${y})"><ellipse cx="${left}" cy="${y}" rx="${rx}" ry="${ry}" fill="${ink}"/><ellipse cx="${right}" cy="${y}" rx="${rx}" ry="${ry}" fill="${ink}"/><g data-heartbound-pupil>${pupilMarkup(face.pupil, left, y + 2, palette.glow)}${pupilMarkup(face.pupil, right, y + 2, palette.glow)}${highlights}</g><path d="M${left - 25} ${y - 45}q25 ${face.brow === "heroic" ? -12 : -5} 50 0M${right - 25} ${y - 45}q25 ${face.brow === "mischievous" ? 11 : -5} 50 0" fill="none" stroke="${palette.accent}" stroke-width="7" stroke-linecap="round"/></g>`;
}
