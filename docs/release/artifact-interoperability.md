...
This document defines the v3/v111 interoperability contract and names the local focused evidence. External writer, strict-live, and remote mutation qualification remain separate gates and are not inferred from package installation or fixtures.

## Authority order

1. The official Receiz v111 verifier establishes cryptographic proof status for the exact enclosing artifact.
2. An Identity Seal may activate only its verified embedded identity. A player Vault supplies canonical identity only with a valid Wildz identity binding or explicit v111 owner-continuity proof. A legacy proof-sealed player Vault supplies artifact-scoped recovery authority, while a card-only Vault never supplies identity authority.
...
| `sdk-compatible` | Local SDK v111 contract coverage; external exercise pending |
