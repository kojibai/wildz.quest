...
test("ARTIFACT-001 through ARTIFACT-030 are pinned to the active v118 constitutional context", () => {
  assert.deepEqual(artifactLaws, Array.from(
    { length: 30 },
    (_, index) => `ARTIFACT-${String(index + 1).padStart(3, "0")}`
  ));
  assert.equal(RECEIZ_V118_REGISTRY_DIGEST, "c284bd39a891c1a828b532523bd548507570819c32e307d79b8043f06d2d3360");
  assert.equal(RECEIZ_V118_APPLICATION_OPERATION_MATRIX_DIGEST, "153b2472830567ec3b445c2c1b4102e4c036ed4c45cc374d40d0079096a40f54");
  ...
  assert.deepEqual({
    admissionIsOperationAuthority: RECEIZ_V118_RELEASE_AUTHORITY.admissionIsOperationAuthority,
    transitionDigestExcludesPlanCoordination: RECEIZ_V118_RELEASE_AUTHORITY.transitionDigestExcludesPlanCoordination,
    effectsDerivedByRegistryOperationLaw: RECEIZ_V118_RELEASE_AUTHORITY.effectsDerivedByRegistryOperationLaw,
    commitDomainNamedAndAtomic: RECEIZ_V118_RELEASE_AUTHORITY.commitDomainNamedAndAtomic,
    receiptIsOperationAuthority: RECEIZ_V118_RELEASE_AUTHORITY.receiptIsOperationAuthority,
    browserAdmissionStoreCarriesProofObjects: RECEIZ_V118_RELEASE_AUTHORITY.browserAdmissionStoreCarriesProofObjects,
    globalMeansNamedCoordinationDomain: RECEIZ_V118_RELEASE_AUTHORITY.globalMeansNamedCoordinationDomain,
    databaseManufacturesArtifactTruth: RECEIZ_V118_RELEASE_AUTHORITY.databaseManufacturesArtifactTruth,
    connectTokenIsArtifactAuthority: RECEIZ_V118_RELEASE_AUTHORITY.connectTokenIsArtifactAuthority,
    offlineDivergenceResolution: RECEIZ_V118_RELEASE_AUTHORITY.offlineDivergenceResolution,
    profileShowcaseArtifactIdentityIsPayloadDigest:
      RECEIZ_V118_RELEASE_AUTHORITY.profileShowcaseArtifactIdentityIsPayloadDigest,
    profileShowcaseLiteralIdentity: RECEIZ_V118_RELEASE_AUTHORITY.profileShowcaseLiteralIdentity,
    profileShowcaseSuccessorHistoryTravelsInSealedBytes:
      RECEIZ_V118_RELEASE_AUTHORITY.profileShowcaseSuccessorHistoryTravelsInSealedBytes,
    profileShowcaseIntroducesNewSignerIssuerOrHeadAuthority:
      RECEIZ_V118_RELEASE_AUTHORITY.profileShowcaseIntroducesNewSignerIssuerOrHeadAuthority,
    nativeCaptureAttestsDedicatedCameraCeremonyOnly:
      RECEIZ_V118_RELEASE_AUTHORITY.nativeCaptureAttestsDedicatedCameraCeremonyOnly,
    pbiAuthorshipRequiresCanonicalEnclosingPredecessor:
      RECEIZ_V118_RELEASE_AUTHORITY.pbiAuthorshipRequiresCanonicalEnclosingPredecessor,
    pbiAuthorshipChangesOwnership: RECEIZ_V118_RELEASE_AUTHORITY.pbiAuthorshipChangesOwnership,
    pbiAuthorshipChangesMediaTruth: RECEIZ_V118_RELEASE_AUTHORITY.pbiAuthorshipChangesMediaTruth,
    pbiAuthorshipAppendsInVerifiedOrder: RECEIZ_V118_RELEASE_AUTHORITY.pbiAuthorshipAppendsInVerifiedOrder,
    offlineSettlementWaitsForGlobalPublication:
      RECEIZ_V118_RELEASE_AUTHORITY.offlineSettlementWaitsForGlobalPublication
  }, {
...
test("the thirty-law custody matrix has executable repository and SDK evidence", () => {
  ...
  const evidence: Record<string, boolean> = {
    ...
    "ARTIFACT-017": RECEIZ_V118_RELEASE_AUTHORITY.recoveryHistoryRequiresIndependentEvidenceRoots,
    "ARTIFACT-018": /challengeB64Url/.test(adapter),
    "ARTIFACT-019": RECEIZ_V118_RELEASE_AUTHORITY.planIdentityDistinctFromAttemptIdentity,
    "ARTIFACT-020": !RECEIZ_V118_RELEASE_AUTHORITY.terminalMcpAttemptConfirmationReusable,
    "ARTIFACT-021": !RECEIZ_V118_RELEASE_AUTHORITY.admissionIsOperationAuthority,
    "ARTIFACT-022": RECEIZ_V118_RELEASE_AUTHORITY.transitionDigestExcludesPlanCoordination,
    "ARTIFACT-023": RECEIZ_V118_RELEASE_AUTHORITY.canonicalIdentityRequiresSigningChallenge,
    "ARTIFACT-024": RECEIZ_V118_RELEASE_AUTHORITY.recoveryHistoryRequiresIndependentEvidenceRoots,
    "ARTIFACT-025": RECEIZ_V118_RELEASE_AUTHORITY.recoveryCommitIsAtomic,
    "ARTIFACT-026": RECEIZ_V118_RELEASE_AUTHORITY.commitDomainNamedAndAtomic,
    "ARTIFACT-027": RECEIZ_V113_GLOBAL_COMMIT_DOMAIN.scheme === "receiz-commit-domain.v1",
    "ARTIFACT-028": /idempotencyKey/.test(adapter),
    "ARTIFACT-029": !RECEIZ_V118_RELEASE_AUTHORITY.receiptIsOperationAuthority,
    "ARTIFACT-030": RECEIZ_V118_APPLICATION_OPERATIONS.includes("artifact.global.resolve")
...
test("v118 MCP and AI Skills expose the same nine-tool artifact operation map", () => {
  ...
  assert.equal(aiIndex.version, "118.0.0");
  assert.equal(aiIndex.registryDigest, RECEIZ_V118_REGISTRY_DIGEST);
  assert.equal(aiIndex.operationMatrixDigest, RECEIZ_V118_APPLICATION_OPERATION_MATRIX_DIGEST);
});
