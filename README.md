...
The application is aligned to the exact Receiz v108 toolchain: `@receiz/sdk@108.0.0`, `@receiz/mcp-server@108.0.0`, and `@receiz/ai-skills@108.0.0`. All three resolve directly from the official npm registry with integrity checks pinned by `pnpm-lock.yaml`. The checked-in `receiz.app.json` is the artifact-first source contract; `pnpm receiz:check` runs the v108 integration checker and locks the release to registry digest `126ca9283fee4ef4c398dbcb958e861cbea191724fdab8eb08df55ff0c14bb79`. The contract forbids database authority, and Wildz adds no external database.
