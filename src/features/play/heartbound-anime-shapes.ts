...
function eyes(p: HeartboundPresentationV3, palette: Palette) {
  ...
  const ink = "#102633";
  const eye = (x: number) => `<g data-heartbound-eye=""><ellipse cx="${x}" cy="${y}" rx="${rx.toFixed(2)}" ry="${ry.toFixed(2)}" fill="#fff8f2" stroke="${ink}" stroke-width="5"/><g data-heartbound-pupil=""><ellipse cx="${x}" cy="${y + 2}" rx="${irisRx.toFixed(2)}" ry="${irisRy.toFixed(2)}" fill="${palette.glow}"/><ellipse cx="${x}" cy="${y + 4}" rx="${(irisRx * .48).toFixed(2)}" ry="${(irisRy * .55).toFixed(2)}" fill="${ink}"/><circle cx="${x + irisRx * .32}" cy="${y - irisRy * .35}" r="${Math.max(5, irisRx * .25).toFixed(2)}" fill="#fff"/><circle cx="${x - irisRx * .2}" cy="${y + irisRy * .12}" r="${Math.max(2.8, irisRx * .12).toFixed(2)}" fill="#fff"/>${p.face.catchlights === 3 ? `<circle cx="${x + irisRx * .24}" cy="${y + irisRy * .35}" r="2.5" fill="#fff"/>` : ""}</g></g>`;
