...
The capture metadata records product commit `c26ae652894db84868c0343c108c048aa32d0fb4` and Next build ID `pVMRsX8Mh21tHuB69B34C`, plus Chromium 152, `http://127.0.0.1:49816/`, 390×844 at DPR 1, and 2026-08-11T05:10:30.321Z–05:10:37.332Z. The validator proves that the recorded product commit exists and is an ancestor of the checked-out release evidence. The build ID is retained capture metadata: a fresh checkout cannot independently attest the untracked `.next` artifact without rebuilding; when `.next/BUILD_ID` is present, validation performs an explicitly optional comparison. The saved replay script installed console, page-error, request-failure, response-status, and request listeners before navigation. It observed exactly 84 requests with 0 console warnings/errors, 0 page errors, 0 failed requests, and 0 HTTP responses ≥400.
...
- Validator: `docs/release/evidence/final-integration/validate-final-integration-evidence.mjs` — executable tracked mode `100755`; the manifest records its computed self SHA-256 and byte size, avoiding a recursive validator/report hash dependency.
...
Exact commands are retained in `manifest.commands`: `pnpm start -p 49816`; cached Playwright CLI fresh-session open; saved `run-code` replay from the tracked script; and `node docs/release/evidence/final-integration/validate-final-integration-evidence.mjs`. The replay deliberately writes a new capture under `output/playwright/`; the tracked JSON and screenshots remain the immutable recorded run. `manifest.capturePathMap` resolves all seven raw path-bearing result fields—the script, result, and five screenshots—to their tracked copies. The validator hard-codes the independent expected literals for browser metadata, timestamps, request count, Profile's exact four-label sequence repeated three times, focus labels, every arrow's coordinates/descendants/named selection, exact before/after state and deltas, pointer transition order, and owner-cancellation fields. It also hard-codes the eight immutable non-validator artifact entries, exact nine-path artifact set, exact capture map, commands, metadata, and claim pointers; the validator self-entry is derived from the tracked validator bytes and requires its exact path and mode. Manifest mutations removing the report or validator, or changing an artifact hash/path/mode, capture mapping, command, or metadata, each fail validation. Product-commit ancestry, Git tracking, SHA-256, byte size, JSON parsing, and clean-checkout validation also pass.
