...
test("Wildz v3 release doctrine names the exact Receiz v113 toolchain", () => {
  ...
  assert.equal(pkg.devDependencies?.["@receiz/mcp-server"], "113.0.0");
  assert.equal(pkg.devDependencies?.["@receiz/ai-skills"], "113.0.0");
  for (const version of ["@receiz/sdk 113.0.0", "@receiz/mcp-server 113.0.0", "@receiz/ai-skills 113.0.0"]) {
    assert.match(release, new RegExp(version.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  }
  assert.match(mcp, /@receiz\/sdk@113\.0\.0/);
  assert.match(mcp, /@receiz\/mcp-server@113\.0\.0/);
  assert.match(mcp, /@receiz\/ai-skills@113\.0\.0/);
