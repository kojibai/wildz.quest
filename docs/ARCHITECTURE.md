...
Creature voice follows the same boundary: the exact card proof and live head remain authority; the server only verifies, observes, transports, and optionally enriches. The local v120 living-subject Twin forms the bounded proof response without network authority. A deterministic source-filter Web Audio instrument constructs the audible voice from the proof signature, birth moment, speaking Kai moment, and response text. Matching Receiz performance may add bounded cadence/emphasis but never replaces that carrier or delays text/memory. Voice playback cannot authorize, block, rewrite, reorder, or erase the existing proof-history conversation append. See [Receiz v120 creature voice](RECEIZ_V120_CREATURE_VOICE.md).
