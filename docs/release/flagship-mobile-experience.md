...
| E16 | Tracked bundle `docs/release/evidence/vault-roster-balanced-hud/`: manifest, executable independent validator, reproducible selection-only browser replay, structured production result, and ten hashed screenshots. It records declared build `9sYHunZv9Fb2Jn8qLGnlF` and product commit `cea7b57` as capture coordinates, then proves exact Toiusap/Neiatid asset ids, full manifest stats, names, portraits, world actors, reload persistence, visible next-battle lead, seven responsive rows, input/recovery, browser health, and disclosed scope boundaries on the served artifact. It does not cryptographically attest source-to-build provenance. |
...
- An existing legitimate two-card production profile proves both selection authorities without making an acquisition claim. Card Vault selected Toiusap (`wilds:123e00f59899025a366d578f`) through its real “Set as active deck leader” control; command name/Stone Pulse, portrait title, world actor, exact active id, and reload all agreed. Slate then selected Neiatid (`wilds:223616f27f33bc5b5fa273d9`); command name/Tide Pulse, portrait title, world actor, exact active id, and reload again agreed. [E16]
- The live Card Vault sealed-manifest DOM recorded full exact stats: Toiusap Health 63 / Power 79 / Guard 76 / Speed 77 / Bond 53; Neiatid Health 87 / Power 61 / Guard 69 / Speed 47 / Bond 55. Through visible Living Story → Battle Trainer controls, Reefway Toma’s selected battle roster then showed Neiatid as Lead and Toiusap as Reserve while the active asset remained Neiatid’s exact id; the locator-level roster remained visibly asserted before and after the screenshot operation. [E16]
- Acquisition/capture provenance is outside this selection replay and is not claimed. The six-second “New” marker is implementation-tested but was not asserted by this browser replay, so it remains automated-only rather than browser-qualified. [E16]
- The balanced HUD replay measured all seven required viewports against one optimized artifact. Every row has a full canvas/drawing buffer/DPR and renderer counters, budget `true`, exactly 14 expected target labels, no undersized target, no safe-bound failure, no painted-surface collision, and zero horizontal overflow. The target contract explicitly includes the Explorer and Mission root buttons; both exceed 44 px in every row. Every named resting screenshot is backed by zero open Slate/live/player/dialog/audio surfaces; preview, expanded Slate, audio, Kai modal, live roster, and empty multiplayer state were then exercised at each size. [E16]
- Recovery passed for simultaneous D-pad/companion touch and release, owned-only horizontal cycle, tap power, named hold-slide ability selection, pointer cancel, lost capture, keyboard selection/focus restoration, resize cancellation, Escape, 200% text, reduced motion, offline/lifecycle, audio toggle/restore, and missing/non-callable/throwing vibration. Browser health recorded zero console warnings/errors, page errors, HTTP errors, and failed requests. [E16]
...
| Vault and Slate authority | Exact Toiusap Card Vault selection and exact Neiatid Slate selection each update command, portrait, world actor, and active asset id and persist through production reload; both full stat blocks and Neiatid’s visible next-battle Lead projection agree | E16 |
| Balanced HUD matrix | 7/7 true resting states, full canvas, renderer budget, exact 14-label target set including Explorer/Mission roots, 44 px floors, safe bounds, painted-surface collisions, overflow, preview/expanded Slate, audio, Kai, live, and empty multiplayer records pass | E16 |
...
4. Both selection authorities and the visible next-battle Lead projection are browser-qualified. The six-second “New” marker is covered only by implementation tests and remains explicitly unqualified in the browser; acquisition provenance is outside this selection replay and is not claimed. [E16]
...
6. The final integration replay had no real retired-card QA profile, so memorial browser replay was not fabricated; memorial ownership, focus containment, Escape, and cleanup are qualified by automated lifecycle contracts only. [E14]
...
5. If acquisition returns to release scope, add a separate legitimate browser replay that asserts the short-lived “New” marker before expiry; do not infer it from this selection-only profile. [Gap: E16]
