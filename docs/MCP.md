...
Wildz pins `@receiz/sdk@105.0.0` as application/runtime authority and `@receiz/mcp-server@105.0.0` plus `@receiz/ai-skills@105.0.0` as development tooling. The finalized packages resolve directly from the official npm registry, and `pnpm-lock.yaml` pins their published integrity values. No fork, patch, third-party database, or external database is introduced.
