...
export function projectWildsResourceAffordance(input: Readonly<{
  ...
  if (input.companionReady === false) return Object.freeze({ state: "rest", label: "Rest companion", guidance: "Pause together before working", enabled: true });
  if (!input.companionQualified) return Object.freeze({ state: "companion", label: noun, guidance: input.kind === "timber" ? "Lead with a woodland worker" : "Lead with a quarry worker", enabled: false });
