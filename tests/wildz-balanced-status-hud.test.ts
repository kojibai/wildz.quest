...
test("balanced homes remain touch-safe and collision-aware at phone and short-landscape sizes", () => {
  ...
  assert.match(finalCss, /\.wilds-map-status-home :is\(\.wilds-live-badge, \.wilds-live-share, \.wilds-live-pill\),[\s\S]*\.wilds-left-instrument-home :is\(\.wilds-kai-command-pill, \.wilds-audio-settings > :is\(button, summary\), \.wilds-message-instrument\)\s*\{[^}]*min-width:\s*44px;[^}]*min-height:\s*44px;/);
