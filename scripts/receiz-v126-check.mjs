...
const TARGET_PACKAGE_VERSION = "126.0.0";
const TARGET_RULESET_VERSION = "126.0.0";
const TARGET_REGISTRY_DIGEST = "80137c2e6f294050ef36ff75e4daac15c7790b7f04d9a91fab9d1970fa3c0b09";
const TARGET_OPERATION_MATRIX_DIGEST = "42c7f0924df91b4ba11c1b891fee2b92abb509430a86b030735c23d055e67949";
const sourceRoot = resolve(process.cwd());
const snapshotRoot = await mkdtemp(join(tmpdir(), "wildz-receiz-v126-check-"));
...
async function assertCompilerBoundary(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    for (const match of source.matchAll(/import\s+(?:type\s+)?\{([^}]*)\}\s+from\s*["']@receiz\/sdk["']/g)) {
      ...
      if (compilerImport) throw new Error(`receiz_v126_compiler_import_on_runtime:${compilerImport}`);
    }
  }
}
...
function assertReleaseIdentity() {
  if (RECEIZ_RELEASE_VERSION !== TARGET_PACKAGE_VERSION || RECEIZ_RULESET_VERSION !== TARGET_RULESET_VERSION) {
    throw new Error("receiz_v126_release_identity_mismatch");
  }
  if (RECEIZ_CURRENT_REGISTRY_DIGEST !== TARGET_REGISTRY_DIGEST
    ...
    throw new Error("receiz_v126_registry_digest_mismatch");
  }
  if (RECEIZ_CURRENT_APPLICATION_OPERATION_MATRIX_DIGEST !== TARGET_OPERATION_MATRIX_DIGEST
    ...
    || RECEIZ_CURRENT_APP_COMPATIBLE_SDK_RANGE !== ">=126.0.0 <127.0.0") {
    throw new Error("receiz_v126_operation_matrix_mismatch");
  }
  if (RECEIZ_CURRENT_CONSTITUTION_REGISTRY.version !== TARGET_RULESET_VERSION
    ...
    throw new Error("receiz_v126_authority_mismatch");
  }
}
...
try {
  ...
  const result = {
    ...
    reviewedV126ScannerFinding: officialResult.blockingFindings.some((finding) => finding.code === reviewedScannerCode)
