...
- Fresh explorer, verified Identity Seal, and verified identity-bearing Vault all use the same automatic Receiz connection gate. A separate dismissible “Connect Receiz” prompt is forbidden.
- Gameplay may mount only when the local actor matches `/api/auth/receiz/me` and canonical-world bootstrap succeeds. Local Practice is reachable only through an explicit offline/recovery choice.
- Keep delegated user access tokens only in the existing scoped HttpOnly cookie. Never write them to browser storage, application state, logs, proof images, or environment variables. `RECEIZ_CONNECT_ACCESS_TOKEN` remains optional and scheduler-only.
- Preserve locally verified identity/Vault state on OAuth cancellation, actor mismatch, bootstrap failure, and network failure. Never replace it with a foreign Receiz actor.
