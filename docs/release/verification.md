...
|---|---|---|---|
| `@receiz/sdk` | `110.0.0` | `110.0.0` | Application identity, artifact, native proof-object, constitutional compiler, command admission, causal replay, checker, and remote-rail client |
| `@receiz/mcp-server` | `110.0.0` | `110.0.0` | Operator tooling; never application authority |
| `@receiz/ai-skills` | `110.0.0` | `110.0.0` | V110 operator procedure guidance; never proof authority |

The finalized v110 SDK, MCP, and AI-skills packages resolve directly from the official npm registry, with their published integrity values pinned by the lockfile. `receiz.app.json` declares artifact-first authority with database authority disabled; the v110 compiler and `pnpm receiz:check` validate the repository integration plan, release identity, ruleset, and registry digest `824aa4af849c4840ba94535798eab36e45d514703b6ae0cd30d4aa53f3c896e4`.

## Local evidence

| Gate | Result |
|---|---|
| `pnpm release:check` | Pass: v110 repository integration and release-identity checker, complete Node test suite, typecheck, lint, full tracked/untracked text secret scan, production build, and default Receiz doctor |
| Historical migration compatibility | Pass: forward-only checkpoint retained; sealed artifacts, receipts, and heads preserved; current execution remains v110-only |
| V110 offline authority | Pass: release identity asserts that a queued proposal is not a global commitment |
...
| Native proof-object continuity | Pass: v110 Record → Seal artifact, owner, record, claim, verify path, Signature V4, exact download digest, and exact-file reopen; `wildz-v110` retries bind to the payload digest |
| Unified artifact recovery | Pass: v110 admission, zero-write recovery planning, verified-capability commit, atomic store boundary, and MCP/AI operation parity are exposed and law-pinned |
...
| Market settlement coordinator | Pass with local contract doubles: admitted trade, Receiz Connect transfer proof, corroborating wallet ledger event, conditional ownership append, and idempotent recovery; v110 exposes no Wildz-specific conditional append, so the live adapter remains fail-closed without that capability |
