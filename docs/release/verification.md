...
Date: 2026-07-30. Target application version: `3.0.0`. Receiz integration target: `116.0.0`.
...
|---|---|---|---|
| `@receiz/sdk` | `116.0.0` | `116.0.0` | Application identity, artifact, native proof-object, constitutional compiler, command admission, causal replay, checker, and remote-rail client |
...
| Gate | Result |
|---|---|
| `pnpm release:check` | Pass: v116 repository integration and release-identity checker, all 959 Node tests, typecheck, MCP conformance, lint, full tracked/untracked text secret scan, production build, and default Receiz doctor |
| Historical migration compatibility | Pass: forward-only checkpoint retained; sealed artifacts, receipts, and heads preserved; current execution remains v116-only |
| V116 offline authority | Pass: a queued proposal is not a global commitment; divergence resolution is structural-only |
...
| Native proof-object continuity | Pass: v116 Record → Seal artifact, owner, record, claim, verify path, Signature V4, exact download digest, and exact-file reopen; `wildz-v116` retries bind to the payload digest |
...
| Profile/economy showcase contract | Pass: all five v116 planning operations are declared; profile identity is literal and owner-scoped, carried successor history remains sealed, no new signer/issuer/head authority is introduced, and economy merge planning requires verified sibling heads |
...
| Market settlement coordinator | Pass with local contract doubles: admitted trade, Receiz Connect transfer proof, corroborating wallet ledger event, conditional ownership append, and idempotent recovery; v116 exposes no Wildz-specific conditional append, so the live adapter remains fail-closed without that capability |
| Bearer claim product flow | Pass: the explicit online action requires the active proof session and player confirmation, submits the complete artifact to the v116 ownership route, independently reopens the returned artifact, downloads the exact admitted bytes, and only then projects verified cards locally; ordinary Vault restore remains independent of this online claim |
