...
test("ARTIFACT-001 through ARTIFACT-030 are pinned to the active v113 constitutional context", () => {
  ...
  assert.equal(RECEIZ_V113_APPLICATION_OPERATION_MATRIX_DIGEST, "091ab9e6b3acb05283510a19754e53c637dbd96b47b499a524dc44c34f8e783b");
  assert.equal(RECEIZ_V113_GLOBAL_COMMIT_DOMAIN.value, "receiz.com/global/v1");
  assert.equal(RECEIZ_V113_PROTOCOL_LIMITS.exactArtifactBytes, 524_288_000);
  assert.equal(RECEIZ_V113_RUNTIME_MATERIALIZATION_LIMITS.exactArtifactBytes, 16_777_216);
  assert.deepEqual({
    admissionIsOperationAuthority: RECEIZ_V113_RELEASE_AUTHORITY.admissionIsOperationAuthority,
    transitionDigestExcludesPlanCoordination: RECEIZ_V113_RELEASE_AUTHORITY.transitionDigestExcludesPlanCoordination,
    effectsDerivedByRegistryOperationLaw: RECEIZ_V113_RELEASE_AUTHORITY.effectsDerivedByRegistryOperationLaw,
    commitDomainNamedAndAtomic: RECEIZ_V113_RELEASE_AUTHORITY.commitDomainNamedAndAtomic,
    receiptIsOperationAuthority: RECEIZ_V113_RELEASE_AUTHORITY.receiptIsOperationAuthority,
    browserAdmissionStoreCarriesProofObjects: RECEIZ_V113_RELEASE_AUTHORITY.browserAdmissionStoreCarriesProofObjects,
    globalMeansNamedCoordinationDomain: RECEIZ_V113_RELEASE_AUTHORITY.globalMeansNamedCoordinationDomain,
    databaseManufacturesArtifactTruth: RECEIZ_V113_RELEASE_AUTHORITY.databaseManufacturesArtifactTruth,
    connectTokenIsArtifactAuthority: RECEIZ_V113_RELEASE_AUTHORITY.connectTokenIsArtifactAuthority,
    offlineDivergenceResolution: RECEIZ_V113_RELEASE_AUTHORITY.offlineDivergenceResolution
  }, {
...
test("the thirty-law custody matrix has executable repository and SDK evidence", () => {
  ...
  const evidence: Record<string, boolean> = {
    ...
    "ARTIFACT-017": RECEIZ_V113_RELEASE_AUTHORITY.recoveryHistoryRequiresIndependentEvidenceRoots,
    "ARTIFACT-018": /challengeB64Url/.test(adapter),
    "ARTIFACT-019": RECEIZ_V113_RELEASE_AUTHORITY.planIdentityDistinctFromAttemptIdentity,
    "ARTIFACT-020": !RECEIZ_V113_RELEASE_AUTHORITY.terminalMcpAttemptConfirmationReusable,
    "ARTIFACT-021": !RECEIZ_V113_RELEASE_AUTHORITY.admissionIsOperationAuthority,
    "ARTIFACT-022": RECEIZ_V113_RELEASE_AUTHORITY.transitionDigestExcludesPlanCoordination,
    "ARTIFACT-023": RECEIZ_V113_RELEASE_AUTHORITY.canonicalIdentityRequiresSigningChallenge,
    "ARTIFACT-024": RECEIZ_V113_RELEASE_AUTHORITY.recoveryHistoryRequiresIndependentEvidenceRoots,
    "ARTIFACT-025": RECEIZ_V113_RELEASE_AUTHORITY.recoveryCommitIsAtomic,
    "ARTIFACT-026": RECEIZ_V113_RELEASE_AUTHORITY.commitDomainNamedAndAtomic,
    "ARTIFACT-027": RECEIZ_V113_GLOBAL_COMMIT_DOMAIN.scheme === "receiz-commit-domain.v1",
    "ARTIFACT-028": /idempotencyKey/.test(adapter),
    "ARTIFACT-029": !RECEIZ_V113_RELEASE_AUTHORITY.receiptIsOperationAuthority,
    "ARTIFACT-030": RECEIZ_V113_APPLICATION_OPERATIONS.includes("artifact.global.resolve")
...
test("v113 MCP and AI Skills expose the same nine-tool artifact operation map", () => {
  ...
  assert.equal(aiIndex.version, "113.0.0");
  assert.equal(aiIndex.registryDigest, RECEIZ_V113_REGISTRY_DIGEST);
  assert.equal(aiIndex.operationMatrixDigest, RECEIZ_V113_APPLICATION_OPERATION_MATRIX_DIGEST);
});
