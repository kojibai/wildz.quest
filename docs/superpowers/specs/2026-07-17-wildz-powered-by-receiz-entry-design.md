...
- Split the entry message into exactly two centered visual lines with distinct hierarchy. The brighter, larger main line is “Catch living creatures shaped by the moment.” The quieter, smaller subtext line is “Train, evolve, breed & carry them anywhere. No two Wildz are ever the same.” Keep each sentence group on its own visual line across supported entry layouts.
...
- Use Receiz’s official 116 × 32 badge artwork as a local asset, rendered at approximately 96 × 26 so it reads as a quieter signature. Preserve a minimum 44px link target around the smaller artwork and an accessible name of “Powered by Receiz”.
- Add approximately 20px more separation above the Receiz link so it sits distinctly below the entry controls and status content without becoming detached from the composition.
...
- Preserve a minimum 44px interactive area around the approximately 96 × 26 rendered badge.
...
- The main statement and subtext remain exactly two visual lines without clipping at the supported viewport widths.
...
- Render the entry route at desktop and narrow mobile sizes, checking the two-line hierarchy, both lines' fit, the smaller badge and increased footer separation, full-page scrolling, badge focus/hover treatment, subtle motion, and the reduced-motion state.
