...
describe("Receiz Wilds rendering contract", () => {
  ...
  it("opens a focus-safe unobstructed 3D atlas", async () => {
    ...
    assert.match(canvas, /ATLAS_MAX_TERRAIN_VERTICES/);
    assert.match(canvas, /ATLAS_MAX_DETAIL_INSTANCES/);
    assert.match(canvas, /event\.delta > ATLAS_CLICK_DRAG_THRESHOLD/);
    assert.match(canvas, /wildsAtlasContainsWorld/);
    assert.doesNotMatch(canvas, /const ATLAS_SCALE = 1\.35/);
    assert.doesNotMatch(canvas, /\* 1\.35/);
  ...
  it("builds the explorer from articulated anatomy and secondary motion", async () => {
    ...
    assert.match(explorer, /length=\{0\.39\} radius=\{0\.095\}/);
    assert.doesNotMatch(explorer, /"leftLeg"/);
    assert.doesNotMatch(explorer, /name="bootSole"/);
    assert.match(explorer, /name="rearHair"/);
    assert.match(explorer, /name="identity-bound-face"/);
    assert.match(explorer, /createWildsExplorerFace/);
  });
  ...
  it("restores the complete vault inside a scrollable mobile command sheet", async () => {
    ...
    assert.match(css, /\.wilds-command-sheet\s*\{[^}]*grid-template-rows:\s*auto minmax\(0, 1fr\)/s);
