...
**Architecture:** Preserve the accepted full-screen standalone UI while migrating proof authority and V3 domain behavior behind focused adapters. Execute five independently reviewable plans in strict dependency order: identity authority, V3 kernel and continuity, player experience, public economy, then production qualification. Each plan leaves the application in a working, testable state and commits only its own reviewed files.

**Tech Stack:** Next.js 15, React 19, TypeScript 5.6, Three.js 0.182, React Three Fiber 9.6, Drei 10.7, `@receiz/sdk` 100.0.0, Node test runner, Playwright/WebKit browser QA, Web App Manifest, service worker, IndexedDB, Web Crypto.

## Global Constraints

- Work on `main`; do not create a release branch and do not push.
- Preserve the current accepted modifications in `app/globals.css`, `PlayCampaign.tsx`, `WildsBattle.tsx`, `WildsCommandDock.tsx`, `WildsCreatureThumbnail.tsx`, `WildsWorldCanvas.tsx`, `WildzSocialDeck.tsx`, `receiz-commerce-vault.ts`, and their four modified tests.
...
| 2 | `2026-07-15-wildz-v3-kernel-continuity.md` | Complete V3 domain kernel, V8 saves, owner-bound combined Vault continuity, canonical world repository, and client API |
| 3 | `2026-07-15-wildz-v3-player-experience.md` | Settlements, ecology, bosses/raids, social/mastery/narrative UI, Trail Pack, complete card rail, and profile sharing in the accepted shell |
| 4 | `2026-07-15-wildz-v3-public-economy.md` | Durable public profiles/cards, authenticated listings/trades, verified settlement, and ownership append |
| 5 | `2026-07-15-wildz-v3-production-release.md` | Offline/update PWA, security, accessibility, performance, release scripts, browser evidence, and final release commit |
...
- `src/lib/receiz/wildz-world-repository.ts` separates canonical Receiz publication from local-practice state.
...
- `src/lib/receiz/wildz-public-repository.ts` owns durable public profiles, listings, trades, ownership, and receipts.
- Production routes derive actors from `receizRequestSession`; request JSON never supplies trusted actor identity.

---

### Task 0: Freeze the Accepted Standalone Baseline

**Files:**
- Verify and commit only: `app/globals.css`
- Verify and commit only: `src/features/play/PlayCampaign.tsx`
- Verify and commit only: `src/features/play/WildsBattle.tsx`
- Verify and commit only: `src/features/play/WildsCommandDock.tsx`
- Verify and commit only: `src/features/play/WildsCreatureThumbnail.tsx`
- Verify and commit only: `src/features/play/WildsWorldCanvas.tsx`
- Verify and commit only: `src/features/play/WildzSocialDeck.tsx`
- Verify and commit only: `src/lib/receiz/receiz-commerce-vault.ts`
- Verify and commit only: `tests/wilds-command-dock.test.ts`
- Verify and commit only: `tests/wilds-render-contract.test.ts`
- Verify and commit only: `tests/wildz-continuity-and-shell.test.ts`
- Verify and commit only: `tests/wildz-social-deck.test.ts`

**Interfaces:**
- Consumes: approved current working-tree UI and Safari-safe SDK fetch correction.
- Produces: a clean, tested preservation checkpoint that later plans merge into rather than replace.

- [ ] **Step 1: Confirm the exact branch and dirty-file set**
...
Expected: branch is `main`; the only uncommitted files are the twelve accepted paths listed above.

- [ ] **Step 2: Check the accepted diff for malformed patches**
...
Expected: `git diff --check` prints nothing; the stat contains no file outside the accepted path list.

- [ ] **Step 3: Re-run the preservation baseline**
...
Expected: 177 Node tests pass with zero failures; typecheck, lint, and secret scan exit 0; doctor reports local identity/artifact/proof rails available and truthfully marks unconfigured remote rails `needs-env`.

- [ ] **Step 4: Stage only the accepted baseline**
...
Expected: exactly the twelve listed files are staged; the approved design and plan documents are already committed separately.

- [ ] **Step 5: Commit the preservation checkpoint**

```bash
git commit -m "feat: preserve refined Wildz mobile gameplay shell"
```

Expected: commit succeeds on `main`; `git status --short` is empty before Plan 1 begins.
...
### Task 2: Execute V3 Kernel and Continuity Plan
...
- Produces: V8 state, complete V3 domain kernel, combined identity-bearing Vault, atomic owner restore, canonical world service/repository, and `{ projection, mode }` client contract.

- [ ] **Step 1: Complete every checkbox in the kernel-continuity plan**

Execute pure domain ports before world orchestration, then V8/Vault continuity, then the atomic server/route/client seam.
...
Expected: all adapted V3 domain tests pass; a generated identity-bearing Vault restores the embedded owner and full V8 state after a cold repository reload; world snapshots have exactly one `projection` layer; the worktree is clean.

---

### Task 3: Execute V3 Player Experience Plan

**Files:**
- Plan: `docs/superpowers/plans/2026-07-15-wildz-v3-player-experience.md`

**Interfaces:**
- Consumes: V3 world client, V8 state, card mastery, narrative, settlement, ecology, raid, and identity projections.
...
### Task 4: Execute Public Economy Plan
...
- Produces: durable public profile/card recovery, authenticated listing/trade/checkout routes, verified settlement receipts, and ownership append.

- [ ] **Step 1: Complete every checkbox in the public-economy plan**

Remove process-memory and client-asserted authority paths. Keep the market embedded and fail closed when remote settlement is unavailable.
...
Expected: public profile/card deep links survive refresh; unauthenticated actors are rejected; payment failure transfers no ownership; admitted settlement creates one idempotent ownership append; the worktree is clean.

---

### Task 5: Execute Production Release Plan

**Files:**
- Plan: `docs/superpowers/plans/2026-07-15-wildz-v3-production-release.md`

**Interfaces:**
- Consumes: the complete functional application from Tasks 1–4.
